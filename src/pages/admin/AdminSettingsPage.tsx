...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="font-display text-2xl font-bold text-text-main mb-6">Site Settings</h1>

      <div className="bg-bg-card rounded-2xl border border-border-main p-6 flex flex-col gap-5">
          ...
          <div className="flex gap-2 flex-wrap mb-2">
            {LOGO_OPTIONS.map(e => (
              <button key={e} onClick={() => setForm(f => ({ ...f, logo: e }))}
                className={`w-10 h-10 rounded-xl text-2xl transition-all ${form.logo === e ? 'bg-brand-gold scale-110' : 'hover:bg-bg-card-alt'}`}>
                {e}
              </button>
            ))}
          </div>
        ...
        <Field label="WhatsApp Number">
          ...
          <p className="text-xs text-text-subtle mt-1">Include country code, digits only preferred</p>
        </Field>
        ...
        <button onClick={handleSave}
          className={`py-3 font-bold rounded-xl transition-colors text-sm ${
            saved ? 'bg-brand-green text-white' : 'bg-brand-gold text-white hover:bg-brand-gold-hover'
          }`}>
